...
def build_adapter_headers(
    ...
    if isinstance(extra_headers, dict) and any(key in extra_headers for key in rule_keys):
        processed = apply_header_rules(base, extra_headers)
        # 恢复受保护键的原值
        ...
        for key in list(processed.keys()):
            if key.lower() in protected and key not in base:
...
def get_extra_headers_from_endpoint(endpoint: Any) -> Optional[Dict[str, Any]]:
    """
    从 endpoint 提取额外请求头
